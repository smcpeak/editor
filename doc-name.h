...
// unique (per `compareTo`) within the list of `NamedTextDocuments` in
// its containing `NamedTextDocumentList`.
class DocumentName {
private:     // data
  // Host and (usually) file name.  The resource name is a file name if
  // `m_hasFilename`.  Otherwise, it is a human-readable string
  // describing the origin of the content, such as a command line that
  // was run, with the document being its output.
  ...
  //
  // If `!empty()`, this must always end with a path separator
...
public:      // methods
  ...
  // Get this as a HARN.
  HostAndResourceName const &harn() const { return m_harn; }

  // Queries on `m_harn`.
  HostName hostName() const            { return m_harn.hostName(); }
  bool isLocal() const                 { return m_harn.isLocal(); }
  bool empty() const                   { return m_harn.empty(); }
  std::string resourceName() const     { return m_harn.resourceName(); }
